...
const JoinClassByCodeModal = ({ isOpen, onClose, callFetchClasses }: EventModalProps) => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        if (!isRegistering) {
            ...
            try {
                const response = await doJoinClassByCode(currentUser?.uid || '', code) as any;
                if (response?.success) {
                    toast.success(<CustomToast title='Congratulation!' subtitle={'You have successfully joined a class.'} />);
                    callFetchClasses();
                    onClose();
                } else {
                    toast.error(<CustomToast title='Failed to join class!' subtitle='Please check the code and try again.' />);
                }
            } catch (error) {
                setErrorMessage('An error occurred while joining the class. Please try again later.');
            } finally {
