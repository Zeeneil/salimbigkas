...
const QuizResult = () => {
  ...
  const { selectedQuiz, results } = useQuizContext();
  const percentage = Math.round((results.score / results.totalQuizScore) * 100);
        ...
        {q.image && (
          <div className="flex justify-center mb-4">
            <img
              ...
              className="rounded-lg shadow-md max-h-48 border-2 border-orange-400"
            />
          </div>
        )}
  ...
  const fullyCorrectCount = useMemo(() => selectedQuiz?.questions.reduce((count: number, q: any) => {
  ...
  const partiallyCorrectCount = useMemo(() => selectedQuiz?.questions.reduce((count: number, q: any) => {
  ...
  // Add Incorrect count
  const incorrectCount = useMemo(() => selectedQuiz?.questions.length - fullyCorrectCount - partiallyCorrectCount, [fullyCorrectCount, partiallyCorrectCount, selectedQuiz?.questions.length]);
  return (
        ...
        >
          ...
          <div className="text-center space-y-4 mb-6">
            <div className="text-[#fff] text-5xl font-extrabold">
              {selectedQuiz?.category || "General Quiz"}
            </div>
          </div>
          ...
          <div className="space-y-6 mt-6">
            ...
            {selectedQuiz?.questions.map((q: any, qIdx: number) => renderQuestionResult(q, qIdx))}
          </div>
