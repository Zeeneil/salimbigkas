...
import { doCreateUser } from '../../api/functions';
...
const AddUser = ({ setShowAddUser, callFetchUsers }: AddUserProps) => {
    ...
    const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!isRegistering) {
          ...
          setErrorMessage('');
          try {
            const response = await doCreateUser(currentUser?.uid || "", fullname, email, password, role) as any;
            if (response?.success) {
                toast.success(<CustomToast title="Congratulation!" subtitle="User registered successfully" />);
                callFetchUsers();
                setShowAddUser();
            } else {
                toast.error(<CustomToast title="Something went wrong!" subtitle="An error occurred while creating the user. Please try again." />);
            }
          } catch (error) {
                ...
                <div className="rounded-xl border-1 border-gray-200 shadow-sm p-8">
                    ...
                    <form onSubmit={handleRegister}>
                        <div className="flex text-left mb-5 justify-between items-center">
                            <h3 className="text-sm text-gray-600 mb-4">Please fill in the details below to register a new user.</h3>
                            {/* Register button */}
                            <button
                                type="submit"
