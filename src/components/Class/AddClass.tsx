...
import { doCreateClass } from '../../api/functions';
...
interface AddClassProps {
    setShowAddClass: () => void;
    callFetchClasses: () => void;
}

const AddClass = ({ setShowAddClass, callFetchClasses }: AddClassProps) => {
    ...
    const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!isRegistering) {
            ...
            try {
                ...
                const response = await doCreateClass(currentUser?.uid || "", classname, classdescription, classgrade, selectedDays, newTime) as any;
                if (response?.success) {
                    toast.success(<CustomToast title="Congratulation!" subtitle="Class added successfully" />);
                    callFetchClasses();
                    setSelectedDays([]);
                    setShowAddClass();
                } else {
                    toast.error(<CustomToast title="Something went wrong!" subtitle="Failed to create class. Please try again." />);
                }
            } catch (error) {
